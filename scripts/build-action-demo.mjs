...
function draw(now){if(now>=nextBlink){blinkUntil=now+220;nextBlink=now+2800+Math.random()*1900}const blend=Math.min(1,(now-transitionStart)/520);const eased=blend*blend*(3-2*blend);ctx.clearRect(0,0,DATA.width,DATA.height);const sway=Math.sin(now/1250)*0.018;const breath=(Math.sin(now/1700)+1)/2;const bob=Math.sin(now/1700)*2.2;ctx.save();ctx.translate(DATA.width/2+bob,DATA.height*.72);ctx.rotate(sway);ctx.scale(1+breath*.002,1+breath*.008);ctx.translate(-DATA.width/2,-DATA.height*.72);for(const layer of [...DATA.layers].reverse()){const a0=opacityFor(layer,previous),a1=opacityFor(layer,state);let a=a0+(a1-a0)*eased;if(/^(irides|eyewhite)/.test(layer.name)&&now<blinkUntil)a=0;ctx.globalAlpha=Math.max(0,Math.min(1,a));ctx.drawImage(imgs.get(layer.name),layer.x,layer.y)}ctx.restore();ctx.globalAlpha=1;requestAnimationFrame(draw)}
