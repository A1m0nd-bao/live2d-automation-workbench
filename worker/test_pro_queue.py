...
class ProTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_resume_and_no_duplicate_generation(self):
        ...
        for state in run['states']:
            self.prep.jobs[state['prep_id']] = {'status': 'succeeded', 'message': 'ok', 'quality': {'status':'passed', 'version':module._quality.VERSION}}
            self.splits[state['split_id']] = {'status': 'succeeded', 'message': 'ok', 'quality': {'status':'passed', 'version':module._quality.VERSION}}
        await self.queue.advance(self.queue.get(run['id']))
        await self.queue.advance(self.queue.get(run['id']))
        self.assertEqual(self.queue.get(run['id'])['status'], 'ready')
        self.assertTrue(all(c[1] == b'\x89PNGbase' for c in self.prep.calls))
    async def test_unreviewed_or_rejected_images_never_split(self):
        run = self.create()
        for state in run['states']:
            self.prep.jobs[state['prep_id']] = {'status':'succeeded', 'message':'old job'}
        await self.queue.advance(run)
        self.assertEqual(run['states'][0]['status'], 'failed')
        self.assertNotIn(run['states'][0]['split_id'], self.splits)
        state=run['states'][1]
        self.prep.jobs[state['prep_id']] = {'status':'needs-review', 'message':'style drift'}
        await self.queue.advance(run)
        self.assertNotIn(state['split_id'], self.splits)
    ...
    async def test_uncertain_never_replayed(self):
        ...
        await self.queue.advance(run)
        second = run['states'][1]
        self.prep.jobs[second['prep_id']] = {'status': 'succeeded', 'message': 'ok', 'quality': {'status':'passed', 'version':module._quality.VERSION}}
        self.splits[second['split_id']] = {'status': 'succeeded', 'message': 'ok', 'quality': {'status':'passed', 'version':module._quality.VERSION}}
        await self.queue.advance(run)
        self.assertEqual(run['status'], 'needs_attention')
        old = first['prep_id']
