...
def process(source, output, padding_ratio=0.0):
    import numpy as np
    ...
    if not 0 <= padding_ratio <= .25:
        raise ValueError('Invalid canvas padding')
    if padding_ratio:
        dx, dy = round(original.width * padding_ratio), round(original.height * padding_ratio)
        padded = Image.new('RGBA', (original.width + dx * 2, original.height + dy * 2))
        padded.paste(foreground, (dx, dy))
        foreground = padded
        report.update(padding=[dx, dy], output_size=list(foreground.size))
    foreground.save(output / "foreground.png")
    foreground.getchannel('A').save(output / "mask.png")
    # Solid neutral gray is explicit: upstream image decoders may discard alpha.
    submission = Image.alpha_composite(Image.new("RGBA", foreground.size, (208,208,208,255)), foreground).convert("RGB")
    submission.save(output / "submission.png")
    Image.alpha_composite(Image.new("RGBA", foreground.size, (40,48,62,255)), foreground).convert("RGB").save(output / "on-dark.png")
...
if __name__ == "__main__":
    ...
    print(json.dumps(process(args.source, args.output, args.padding_ratio)))
